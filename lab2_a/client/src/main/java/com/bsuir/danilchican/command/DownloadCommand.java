...
class DownloadCommand extends AbstractCommand {
    ...
    private static final byte EXPECTED_URGENT_BYTE = (byte) 'm';
                    ...
                    if (connection.sendMessage(START_TRANSFER)) {
                        ...
                        try {
                            ...
                            while ((count = connection.receive(buff)) != -1) {
                                byte[] tempData = Arrays.copyOfRange(buff, 0, count);
                                ...
                                if (urgentPos != -1) {
                                    ...
                                    if (urgentPos == 0 && tempData.length > 1) {
                                        tempData = Arrays.copyOfRange(tempData, 1, tempData.length);
                                    } else if (tempData.length > 1) {
                                        tempData = Arrays.copyOfRange(tempData, 0, tempData.length - 1);
                                    }

                                }

                                receivedBytes += tempData.length;
                                fos.write(tempData);
    ...
    private int hasUrgentData(byte[] buff) {
        int urgentPos = -1;

        for (int i = 0; i < buff.length; i++) {
            if (buff[i] == EXPECTED_URGENT_BYTE && urgentPos == -1) {
                urgentPos = i;
            } else if (urgentPos != -1) {
                buff[i - 1] = buff[i];
            }
